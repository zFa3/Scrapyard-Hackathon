...
// Create a software serial object on pins 10 and 11 (for communication with Arduino 1)
SoftwareSerial mySerial(2, 3);  // RX, TX for communication with Arduino 1
...
void setup() {
  ...
  Serial.begin(9600);   // Communication with Computer 2
...
void loop() {
  ...
  // Send the velocity data to Arduino 1 (via SoftwareSerial)
  ...
  if (millis() - lastSent > interval) {
    ...
    if (Serial.available() > 0) {
      ...
      mySerial.flush();  // Clears the serial buffer
    }
  }
